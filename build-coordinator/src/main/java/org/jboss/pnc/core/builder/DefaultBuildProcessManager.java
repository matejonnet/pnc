 ...
 */
@ApplicationScoped
public class DefaultBuildProcessManager implements BuildProcessManager {

    private final Logger log = LoggerFactory.getLogger(DefaultBuildProcessManager.class);
...
//    private ManagedThreadFactory threadFactory;
    ...
    @Inject
    public DefaultBuildProcessManager(DatastoreAdapter datastoreAdapter, RepositoryManagerFactory repositoryManagerFactory, BuildDriverFactory buildDriverFactory, EnvironmentDriverFactory environmentDriverFactory) {
    ...
    public void processBuildTask(BuildTask buildTask, Runnable onComplete) {
        try {
            startBuilding(buildTask, onComplete);
    ...
    private boolean isConfigurationBuilt(BuildConfiguration buildConfiguration) {
        return datastoreAdapter.isBuildConfigurationBuilt(); //TODO how do we check for existing builds
    }

    private void startBuilding(BuildTask buildTask, Runnable onComplete) throws CoreException {
    ...
    private BuildRecord storeResults(BuildTask buildTask, BuildResult buildResult, Runnable onComplete, Throwable e) {
